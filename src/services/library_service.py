...
class LibraryService:
    ...
    @staticmethod
    def search_books_by_title(user_id, search_query, exact_match=False):
        """
        ...
        :param exact_match: Determines the search mode. If True, searches for titles containing the search query as a
        standalone word. If False, searches for titles containing the search query as part of any word.
        ...
        try:
            ...
            if exact_match:
                # Constructing patterns for the title as a standalone word, at the start, at the end, or is the title
                like_pattern = f"% {search_query} %"  # Word somewhere in the middle
                start_pattern = f"{search_query} %"  # Word at the start
                end_pattern = f"% {search_query}"  # Word at the end
                exact_pattern = search_query  # Exact word

                # Single query with a more streamlined approach
                books_query = """
                            ...
                            WHERE b.title_without_series LIKE %s OR
                                  b.title_without_series LIKE %s OR
                                  b.title_without_series LIKE %s OR
                                  b.title_without_series = %s
                        """
                cursor.execute(books_query, (user_id, like_pattern, start_pattern, end_pattern, exact_pattern))

            else:
                # regex search to match title (collation issues)
                books_query = f"""
                                SELECT b.book_id, b.title_without_series, b.publication_year, b.publisher, b.average_rating, b.image_url, b.url, b.num_pages,
                                ul.book_id IS NOT NULL AS user_has_book
                                FROM books b
